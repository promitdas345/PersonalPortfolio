...
function createLoaders({ postsFile, projectsFile, pacmanSectionFile, analyticsPartialFile }) {
  ...
  let mongoDbLoaders = null;

  if (useMongoDb) {
    try {
      mongoDbLoaders = require('./data-mongodb');
      console.log('✅ Using MongoDB for blog posts');
    } catch (error) {
      console.warn('⚠️  MongoDB enabled but failed to load module:', error.message);
  ...
  async function loadPosts(options = {}) {
    ...
    if (useMongoDb && mongoDbLoaders) {
      try {
        const mongoPosts = await mongoDbLoaders.loadPosts(options);
        return mongoPosts.map(toRuntimePost);
      } catch (error) {
        console.warn('⚠️  MongoDB query failed, falling back to JSON file storage:', error.message);
      }
    }
  ...
  async function updatePosts(mutator) {
    // Use MongoDB if available
    if (useMongoDb && mongoDbLoaders) {
      return await mongoDbLoaders.updatePosts(mutator);
    }
