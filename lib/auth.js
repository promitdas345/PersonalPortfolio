...
function createAuthSystem(config) {
  ...
  // REMOVED: const adminSessions = new Map(); 
  ...
  async function loadAdminAuthRecord() {
    ...
    try {
      ...
      // Return with isNew flag so server.js can log the password
      return { ...defaultRecord, isNew: true };
    }
  }
  ...
  function clearExpiredSessions(store) {
    const now = Date.now();
  ...
  async function getSession(req) {
    ...
    const session = (store.sessions || []).find(s => s.tokenHash === th);

    if (!session) return null;

    if (session.expiresAt <= Date.now()) {
      mutateStore(s => {
        if (!s.sessions) return { result: null };
        s.sessions = s.sessions.filter(item => item.tokenHash !== th);
        return { result: null };
      }).catch(err => console.error('Failed to cleanup expired session', err));
      return null;
    }
    ...
    if (Date.now() - lastSeen > 5 * 60 * 1000) {
      mutateStore(s => {
        const target = (s.sessions || []).find(item => item.tokenHash === th);
  ...
  async function deleteSession(token) {
    ...
    const th = hashToken(token);
    await mutateStore(store => {
      if (!store.sessions) return { result: null };
  ...
  async function upgradePasswordHashIfLegacy(userId, password) {
    const authRecord = await loadAdminAuthRecord();
    const parsed = parsePasswordHash(authRecord.passwordHash);
    if (!parsed || parsed.version !== 'v1') return;
    const newHash = createPasswordHash(password);
    await writeAdminAuthRecord({ ...authRecord, passwordHash: newHash, updatedAt: new Date().toISOString() });
    await mutateStore(store => {
      const target = store.users.find(u => u.id === userId);
      if (target && parsePasswordHash(target.passwordHash)?.version === 'v1') {
